    ...
    >
      ...
      >
        <template v-if="(isCollapsed && !isFirstLevel) || !isCollapsed || isMobileItem">
          <span class="vsm--title">{{ isCollapsed && isFirstLevel ? '' : (itemHasName(item) ? item.name : item.children[0]['name']) }}</span>
        </template>
      </transition>
      <template v-if="(isCollapsed && !isFirstLevel) || !isCollapsed || isMobileItem">
        <sidebar-menu-badge
...
export default {
  ...
  computed: {
    ...
    itemLinkAttributes () {
      ...
      const tabindex = this.item.disabled ? -1 : undefined
    ...
    itemHasChild () {
      return !!('name' in this.item && this.item.children && this.item.children.length > 0)
    }
  },
  watch: {
    $route () {
  ...
  methods: {
    ...
    matchRoute ({ href, exactPath }) {
      ...
      if (this.$router) {
        ...
        return exactPath ? route.fullPath === this.$route.path : this.matchExactRoute(href)
      } else {
    ...
    clickEvent (event) {
      ...
      let cond = !this.item.href
      if (!this.itemHasName(this.item)) {
        cond = !this.item.children[0]['href']
      }
      if (cond) {
